# Name: ZMobile Pain Coach Clinician (Terremark)

Business and Application Owers:  Kathy Frisbee
...
ITOPs to review if "obsolete" system based on usage     |     | CRITICAL task; obsolete mobile apps will not be migrated
ECSO Migration wave planning started              |     |  
Baseline costs (pre-cloud) documented             |      | 
Validated ACTIVE users / traffic analytics for past 12 months   |    | CRITICAL task for mobile apps; what is status of Patty C. team doing this analysis?
...
Support contracts for application teams awarded (as needed) / resource onboarding completed (GFE, PIV, VA account set up, etc.) |.     |             
