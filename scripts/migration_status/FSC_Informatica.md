...
Dev/Test/Staging/Prod environment set up completed                 | Red      | Resource groups for environment are set up blocked on setting up VMs (self-service provisioning for creating VMs is no longer possible (ITOPS is blocking), MS team trying to solve but unable to date, etc)
